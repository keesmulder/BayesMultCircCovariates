...
shpd <- select(dpd, ppid, imdev_signed, deldev_signed, CODE)
longpd <- gather(shpd, delay, dev, -CODE, -ppid)
longpd$delay <- factor(longpd$delay, labels = c("10s", "2s"))
longpd$ppid  <- factor(longpd$ppid)

# "No significant effect of delay"
del_aov <- aov(dev ~ CODE*delay + Error(ppid/delay), data = longpd)
summary(del_aov)
