...
opts_chunk$set(fig.path='figure/', fig.align='center', fig.show='hold', fig.height=4, fig.width=10, echo=FALSE, warning=FALSE, message=FALSE)
...
Another example dataset is given in Figure \ref{ZeroBData}. Here, true $\beta$ is zero, and the predictor $X$ consists of a sample with $n=30$ from the normal distribution that was standardized. Once again, true $\beta_0 = \pi$.
...
grid.arrange(ll1p1, ll2p1, ll3p1, ncol=3)
