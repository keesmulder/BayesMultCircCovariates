...
b0cur <- 0
xl <- 30 * c(-1, 1)
ylb <- expression(paste("Conditional posterior of ", beta))

ll1p1 <- plotbeta(normalPrior=TRUE, res=res, xl=xl, b0=0,
                  kp=1, r=2, th=th, X=X) +
  myTheme() + labs(title=expression(paste(beta[0], " = 0")), y = ylb)
ll2p1 <- plotbeta(normalPrior=TRUE, res=res, xl=xl, b0=pi/2,
                  kp=1, r=2, th=th, X=X) +
  myTheme() + labs(title=expression(paste(beta[0], " = ", pi / 2)), y = ylb)
ll3p1 <- plotbeta(normalPrior=TRUE, res=res, xl=xl, b0=pi,
                  kp=1, r=2, th=th, X=X) +
  myTheme() + labs(title=expression(paste(beta[0], " = ", pi)), y = ylb)
...
% \texttt{For each $\beta_j$, select an appropriate bandwith $b_j.$}
 ...
 \For{$i \leftarrow 2$ \KwTo $Q$}{
  ...
  $\psi_i = \theta_i + \boldsymbol{\delta} \boldsymbol{z}_i  - g (\boldsymbol{\beta}^T \boldsymbol{x}_i),$ for each $i = 1, \dots, n.$
...
load(paste0("Simulation/Results/[simStudCircGLM_nsim1000_Q20000_burnin1000_r2_bt_prior1_seed389238_n20,50,100_kp1,4,32_btl,c,lll,ccc,llllll,cccccc,0.05,0.3].rda"))

# print.cGLMSim(simStudyResults)

# print.cGLMSim(simStudyResults[1], selection = list('n', 'kp',
